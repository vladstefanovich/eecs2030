...
public class DictionaryTest {
	...
	private static final List<String> WORDS = 
			Collections.unmodifiableList(DictionaryTest.readDictionary());
	...
	public void test00_ctor() {
		assertEquals("can't read dictionary.txt",
				NUMBER_OF_WORDS, WORDS.size());
		
		Dictionary d = new Dictionary();
		assertEquals("dictionary has the wrong number of words",
				NUMBER_OF_WORDS, d.size());
	}
	...
	public void test01_lookUp() {
		...
		for (int i = 0; i < N; i++) {
			...
			assertTrue(word + " is in the dictionary but lookUp returned false",
					d.lookUp(word));
	...
	public void test02_lookUp() {
		...
		for (int i = 0; i < NOT_WORDS.length; i++) {
			...
			assertFalse(word + " is not in the dictionary but lookUp returned true",
					d.lookUp(word));
	...
	public void test04_wordsStartingWith() {
		...
		final String[][] EXP = {
				{"cageless", "cagelike", "cageling", "cagelings"},
				{"pricier", "priciest", "pricing"},
				{"thesis"},
		...
		for (int i = 0; i < PREFIX.length; i++) {
			...
			assertEquals("wordsStartingWith(\"" + PREFIX[i] + "\") failed",
					exp, d.wordsStartingWith(PREFIX[i]));
		}
	}
}
